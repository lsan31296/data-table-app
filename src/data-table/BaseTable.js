...
function BaseTable({ tableData, handleSearch }) {
    const columnHeaders = [
        ...
        { name: 'Loan Amount', selector: row => row.loan_amount, sortable: true, format: row => formatter.format(row.loan_amount) },
        { name: 'State Code', selector: row => row.state_code, sortable: true },
        { name: 'Market Value', selector: row => row.market_value, sortable: true, format: row => formatter.format(row.market_value) },
        { name: 'Pool Number', selector: row => row.pool_number, sortable: true },
        { name: 'Fund Ticker', selector: row => row.fund_ticker, sortable: true },
        { name: 'Security Group', selector: row => row.security_group, sortable: true },
        { name: 'Security Type', selector: row => row.security_type, sortable: true },
        { name: 'Security Sector', selector: row => row.security_sector, sortable: true },
        { name: 'Weight', selector: row => row.weight, sortable: true, format: row => row.weight.toFixed(4) },
    ]
    ...
    */
    ...
    return (
        <div style={{ padding: "5% 4%", backgroundColor: '#F2F2F2' }}>
            <div style={{ display: "flex", justifyContent: "space-between", backgroundColor: "#1B3668", color: "white", padding: "2px" }}>
